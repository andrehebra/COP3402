...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < lineCount; i++) {
        ...
        if (caught == 0) {
            int stop = 0;
            while (strlen(lineByLine[i]) > 0 || stop != 1) {
                char reservedWord[200];
                strcpy(reservedWord, getReserved(lineByLine[i], reservedWord));

                if (strcmp(reservedWord, "NEG") == 0) {

                } else {
                    if (strcmp(reservedWord, "const") == 0) {
                        removeWord(lineByLine[i], "const");
                        strcpy(tokenList[tokenPosition++], "28");
                        printf("\n%-10s 28", "const");
                        stop = 1;
                    } else if (strcmp(reservedWord, "procedure") == 0) {
